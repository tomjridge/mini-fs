...
let mk_msg_ops (type t) 
    ~(call:msg_from_client -> (msg_from_server',t) m)
    ~data_length
    ~data_of_buffer
    ~blit_data_to_buffer
  = 
  let root : path = failwith "FIXME" in
  let ret_unit = bind @@ function
    | Unit -> return ()
    | _ -> err __LOC__
  in
  let unlink ~parent ~name = Unlink(parent,name) |> call |> ret_unit in
  let mkdir ~parent ~name = Mkdir(parent,name) |> call |> ret_unit in
  let opendir p = Opendir(p) |> call |> bind @@function
    | Dh dh -> return dh
    | _ -> err __LOC__
  in
  let readdir dh = Readdir(dh) |> call |> bind @@ function
  ...
  let open_ p = Open(p) |> call |> bind @@ function
    | Open' fd -> return fd
    | _ -> err __LOC__
  in
  let pread ~fd ~foff ~length ~buffer ~boff =
    Pread(fd,foff,length) |> call |> bind @@ function
    | Pread' data -> 
  ...
  let pwrite ~fd ~foff ~length ~buffer ~boff =
    ...
    Pwrite(fd,foff,data) |> call |> bind @@ function
  ...
  let stat_file p = Stat_file p |> call |> bind @@ function
    | Stat_file' sz -> return {sz}
    | _ -> err __LOC__
  in
  let kind p = Kind p |> call |> bind @@ function
    | Kind' k -> return k
    | _ -> err __LOC__
  in
