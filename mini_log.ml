...
let mk_logged_ops (type m) ~log_op ~ops = 
  ...
  let unlink ~parent ~name = log_op.log (Unlink(parent,name)) (unlink ~parent ~name) in
  ...
  let create ~parent ~name = log_op.log (Create(parent,name)) (create ~parent ~name) in
  ...
  let pwrite ~fd ~foff ~length ~buffer ~boff =
    (* FIXME log_op.log (Pwrite(fd,foff,data???)) *) (pwrite ~fd ~foff ~length ~buffer ~boff) in
  let close fd = log_op.log (Close(fd)) (close fd) in
  let truncate ~path ~length = log_op.log (Truncate(path,length)) (truncate ~path ~length) in
